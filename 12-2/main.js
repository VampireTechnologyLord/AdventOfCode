...
fs.readFileSync("./12-2/input.txt", "utf8")
    .split("\n")
    .forEach((line) => {
        lines.push(line);
    });
...
function getCorners(area) {
    let corners = 0;
    const value = area.value;
    area.cells.forEach((cell) => {
        if (cell.borders.top && cell.borders.left) {
            corners++;
        }
        if (cell.borders.top && cell.borders.right) {
            corners++;
        }
        if (cell.borders.bottom && cell.borders.left) {
            corners++;
        }
        if (cell.borders.bottom && cell.borders.right) {
            corners++;
        }

        const topRight = grid.find((c) => c.x === cell.x + 1 && c.y === cell.y - 1);
        const topLeft = grid.find((c) => c.x === cell.x - 1 && c.y === cell.y - 1);
        const bottomRight = grid.find((c) => c.x === cell.x + 1 && c.y === cell.y + 1);
        const bottomLeft = grid.find((c) => c.x === cell.x - 1 && c.y === cell.y + 1);

        if (topRight) {
            if (topRight.value === value) {
                if (cell.borders.right && topRight.borders.bottom) {
                    corners++;
                }
            }
        }
        if (topLeft) {
            if (topLeft.value === value) {
                if (cell.borders.left && topLeft.borders.bottom) {
                    corners++;
                }
            }
        }
        if (bottomRight) {
            if (bottomRight.value === value) {
                if (cell.borders.right && bottomRight.borders.top) {
                    corners++;
                }
            }
        }
        if (bottomLeft) {
            if (bottomLeft.value === value) {
                if (cell.borders.left && bottomLeft.borders.top) {
                    corners++;
                }
            }
        }
...
areas.forEach((area) => {
    const corners = getCorners(area);
    const areaSize = countArea(area);
    console.log(`Area of ${area.value} has ${corners} corners and ${areaSize} cells - Thus fencing it costs ${corners * areaSize}`);
    price += corners * areaSize;
    fs.writeFileSync(`./12-2/output.txt`, `Area of ${area.value} has ${corners} corners and ${areaSize} cells - Thus fencing it costs ${corners * areaSize}\n`, {flag: "a"});
})
