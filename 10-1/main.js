...
fs.readFileSync("./10-1/input.txt", "utf8")
    .split("\n")
    .forEach((line) => {
        lines.push(line);
    });
...
const xyPairs = validPaths.map((path) => path.map((cell) => [cell.x, cell.y]));

const uniquePaths = xyPairs.filter((path, index) => {
    for (let i = 0; i < xyPairs.length; i++) {
        if (i !== index) {
            if (JSON.stringify(path) === JSON.stringify(xyPairs[i])) {
                return false;
            }
        }
    }
...
console.log(uniquePaths.length); // 794
